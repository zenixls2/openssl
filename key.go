...
// LoadPrivateKeyFromEngine loads a private key from an OpenSSL ENGINE
func LoadPrivateKeyFromEngine(e *Engine, key_id []byte) (PrivateKey, error) {
	key := C.X_ENGINE_load_private_key(e.e, (*C.char)(unsafe.Pointer(&key_id[0])), nil, nil)
	if key == nil {
...
// LoadPublicKeyFromEngine loads a public key from an OpenSSL ENGINE
func LoadPublicKeyFromEngine(e *Engine, key_id []byte) (PrivateKey, error) {
	key := C.X_ENGINE_load_public_key(e.e, (*C.char)(unsafe.Pointer(&key_id[0])), nil, nil)
	if key == nil {
		return nil, errors.New("Could not load private key from engine")
	}
